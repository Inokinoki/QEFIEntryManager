    ...
    : QWidget(parent)
{
    ...
    m_topLevelLayout->addRow("Device Path type:",
                             new QLabel(convert_device_path_type_to_name(dp->type()) + " " + convert_device_path_subtype_to_name(dp->type(), dp->subType())));
    m_topLevelLayout->addRow(QStringLiteral(""),
                             new QLabel(QStringLiteral("%1 %2").arg(dp->type(), 2, 16, QLatin1Char('0')).arg(dp->subType(), 2, 16, QLatin1Char('0'))));
